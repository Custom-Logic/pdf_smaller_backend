...
class CleanupService:
    ...
    def get_cleanup_statistics() -> Dict[str, Any]:
        ...
        try:
            session_jobs = Job.query.filter_by(session_id=session_id).all()
            ...
            for job in session_jobs:
                try:
                    ...
                    error_msg = f"Error cleaning up job {job.job_id} for session {session_id}: {str(e)}"
                    logger.error(error_msg)
                    cleanup_summary['errors'].append(error_msg)

            db.session.commit()

            logger.info(f"Force cleanup for session {session_id}: {cleanup_summary['jobs_cleaned']} jobs cleaned")

        except Exception as e:
            db.session.rollback()
            error_msg = f"Error during force cleanup for session {session_id}: {str(e)}"
            logger.error(error_msg)
            cleanup_summary['errors'].append(error_msg)
